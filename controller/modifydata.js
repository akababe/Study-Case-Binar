...
const createData = async(req, res) => {
  const { mat_desc, mat_in, mat_out, rak, vendor } = req.body
  const maxId = await pool.query('SELECT max(mid) FROM m_stock')
  const inputId = maxId.rows[0].max + 1

  pool.query('INSERT INTO m_stock (mid, mat_desc, mat_in, mat_out, rak, vendor) VALUES ($1, $2,$3,$4,$5,$6)', [inputId, mat_desc, mat_in, mat_out, rak, vendor], (error, results) => {
    if (error) {
      throw res.status(400).json(error)
    }
...
const updateData = (req, res) => {
    ...
    pool.query(
      'UPDATE m_stock SET mat_desc =$1, mat_in=$2, mat_out=$3, rak=$4, vendor=$5 WHERE mid=$6',
      [mat_desc, mat_in, mat_out, rak, vendor, mid],
      (error, results) => {
        if (error) {
          throw res.status(400).json(error)
        }
